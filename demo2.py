...
def main():
    ...
    output_dir = os.path.join(args.output_dir, sequence)
    os.makedirs(output_dir, exist_ok=True)

    out_fn = os.path.join(output_dir, "out.json")
    # json file
    out_dict = {}

    cap = cv2.VideoCapture(args.video)
    assert cap.isOpened(), f"Faild to load video file {args.video}"
    ...
    for idx in tqdm(range(length)):
        ...
        for batch in dataloader:
            ...
            for n in range(batch_size):
                ...
                # cv2.imwrite(os.path.join(output_dir, f'{img_fn}_{person_id}.png'), 255*final_img[:, :, ::-1])
                ...
                #     tmesh.export(os.path.join(output_dir, f'{img_fn}_{person_id}.obj'))
